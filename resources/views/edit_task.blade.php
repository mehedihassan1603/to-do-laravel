...
<body class="bg-gray-100">
    <div class="container mx-auto py-8">
        ...
        <form action="{{ route('update_task') }}" method="POST">
            @csrf
            <input type="hidden" value="{{ $task_info->id }}" name="task_id">
            <div class="mb-4">
                <label for="name" class="block font-semibold">Name:</label>
                <input type="text" id="name" name="name" required value="{{ $task_info->name }}" class="border border-gray-300 rounded-md px-3 py-2 w-full">
            </div>
            <div class="mb-4">
                <label for="father_name" class="block font-semibold">Father's Name:</label>
                <input type="text" id="father_name" name="father_name" required value="{{ $task_info->father_name }}" class="border border-gray-300 rounded-md px-3 py-2 w-full">
            </div>
            <div class="mb-4">
                <label for="mother_name" class="block font-semibold">Mother's Name:</label>
                <input type="text" id="mother_name" name="mother_name" required value="{{ $task_info->mother_name }}" class="border border-gray-300 rounded-md px-3 py-2 w-full">
            </div>
            <div class="mb-4">
                <label for="phone_number" class="block font-semibold">Phone Number:</label>
                <input type="text" id="phone_number" name="phone_number" required value="{{ $task_info->phone_number }}" class="border border-gray-300 rounded-md px-3 py-2 w-full">
            </div>
